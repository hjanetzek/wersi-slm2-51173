...
LABELS = {
    ...
    0x023A: ("init_full", "RESET / INITIALIZATION"),
    0x023C: ("init_srp", "Also called by op_conditional_counter / WAIT (NOP?)"),  # SRP #$00
    0x0250: ("init_enable_irq", "Also called by cmd_stop"),  # CLR IMR; EI; DI; LD IPR; LD IMR; CLR IRQ; EI
    0x025D: ("spin_forever", None),
    0x025F: ("ldei_chain_start", "Start of LDEI bulk copy"),
    0x02E0: ("irq3_handler", "IRQ3 — RAUD Command Dispatch"),
    0x0302: (".cmd_init", "First-time setup"),
    0x0326: (".cmd_dispatch", "Command dispatch"),
    0x0330: (".cmd_run_prog", "RAUD default — reload micro-program pointer + pitch envelope from slave RAM"),
    0x0374: (".cmd_voice_param_update", "VOICE PARAMETER UPDATE"),
    0x0389: (".cmd_stop", "STOP — Voice off"),
    0x03A5: (".cmd_full_setup", "FULL VOICE SETUP"),
    0x0412: ("synthesis_mode_c", "Mode C setup — wrap-point division + wavetable fill ($F2). MODE bits 5:4 = $10"),
    0x0447: (".div_bit1", None),
    0x0457: (".div_bit2", None),
    0x0467: (".div_bit3", None),
    0x0477: (".div_bit4", None),
    0x0487: (".div_bit5", None),
    0x0497: (".div_bit6", None),
    0x04A7: (".div_bit7", None),
    0x04B7: (".div_done", None),
    0x04C6: (".fill_loop", None),
    0x04DD: ("synthesis_mode_b", "Mode B setup — waveform load + variable-duty DDS ($CF). MODE bits 5:4 = $00"),
    0x04EE: (".wave_copy_loop", None),
    0x0502: (".wave_ldei_path", None),
    ...
    0x057B: ("synthesis_loop_entry", "Synthesis loop entry — CLR ACC, add $1D to PROG_CTR, enter interpreter"),
    0x058E: ("micro_program_interpreter", "Micro-program interpreter"),
    0x05BC: ("op_load_r8r9", "ENV=imm ($x1) — load immediate to R8:R9"),
    0x05CA: ("op_load_1617", "AB=imm ($x2) — load immediate to R16:R17"),
    0x05D7: ("op_load_1415", "CD=imm ($x3) — load immediate to R14:R15"),
    0x05E4: ("op_copy_1617_to_r8r9", "ENV=AB ($44) — copy R16:R17 → R8:R9"),
    0x05EB: ("op_copy_1415_to_r8r9", "ENV=CD ($48) — copy R14:R15 → R8:R9"),
    0x05F2: ("op_store_r8r9_to_1617", "AB=ENV ($54) — copy R8:R9 → R16:R17"),
    0x05F8: ("op_copy_1415_to_1617", "AB=CD ($4C) — copy R14:R15 → R16:R17"),
    0x0600: ("op_store_r8r9_to_1415", "CD=ENV ($58) — copy R8:R9 → R14:R15"),
    0x0606: ("op_copy_1617_to_1415", "CD=AB ($5C) — copy R16:R17 → R14:R15"),
    0x060F: ("op_regpair_op_08", "ENV MEM ($A4) — register pair indirect op on R8:R9"),
    0x0613: ("op_regpair_op_16", "AB MEM ($A8) — register pair indirect op on R16:R17"),
    0x0617: ("op_regpair_op_14", "CD MEM ($AC) — register pair indirect op on R14:R15"),
    0x0677: (".regpair_exit", None),
    0x0680: ("op_add_imm_to_r8r9", "ENV+=imm ($x5) — add immediate to R8:R9"),
    0x06B2: ("op_add_1617_to_r8r9", "ENV+=AB ($64) — add R16:R17 to R8:R9"),
    0x072C: ("op_neg16_multiply", "ENV*=0.(-A) ($F0) — negate R16_HI then multiply ENV"),
    0x074D: ("multiply_8x16_r8r9", "8x16 multiply variants"),
    0x077A: ("op_mul14_to_1617", None),
    0x07E9: ("multiply_acc_r8r9", None),
    0x086D: ("op_mul_1617_by_imm", "AB=0.imm*(A+B) ($B0) — fractional scale of R16:R17 sum"),
    0x08CD: ("cmp_s_r8r9_imm", "IF ENV>imm GOTO ($x9) — signed compare ENV vs immediate"),
    0x0968: ("branch_dispatch", "GOTO ($70) — unconditional branch dispatch"),
    0x097B: ("op_computed_jump", "Computed micro-program jump (GOTO with computed target)"),
    0x09C8: ("op_loop_primary", "LOOP0 N=imm ($04) — primary loop, counter in LOOP1_CTR"),
    0x09E3: ("op_loop_secondary", "LOOP1 N=imm ($14) — secondary loop, counter in LOOP2_CTR"),
    0x0A2D: ("op_conditional_counter", "WAIT ($00) — conditional counter / timed delay"),
    0x0A58: ("op_swap_1415_1617", "EXCHG AB CD ($60) — swap R14:R15 ↔ R16:R17"),
    0x0A69: ("op_neg_1617", "AB=-AB ($40) — negate R16:R17 (two's complement)"),
    0x0AC3: ("op_vibrato1", "VIBRATO1 ($80) — alternating coefficient multiply, 4-byte"),
    0x0AEB: (".vib1_use_coeff2", None),
    0x0AF1: ("op_vibrato2", "VIBRATO2 ($90) — sine phase modulation, 4-byte"),
    ...
    0x0B94: ("op_lfsr_noise", "NOISE ($10) — 16-bit Galois LFSR pitch modulation, 4-byte"),
    ...
    0x0C41: ("op_lfsr_mul_1617", "AB=NOISE X=imm ($20) — LFSR step + multiply → R16:R17"),
    0x0C73: ("op_lfsr_mul_1415", "CD=NOISE X=imm ($30) — LFSR step + multiply → R14:R15"),
    0x0CA5: ("finalize_output", "ACC library init + 12x12 unsigned multiply + pitch envelope + pitch output"),
    ...
    0x0E19: (".f2_nyquist_gate", None),  # Mode C anti-aliasing: silence when T*2 >= R15
    ...
    0x0690: ("op_add_imm_to_1617", "AB+=imm ($x6) — add immediate to R16:R17"),
    0x06A1: ("op_add_imm_to_1415", "CD+=imm ($x7) — add immediate to R14:R15"),
    0x06BB: ("op_add_1415_to_r8r9_sat", "ENV+=CD sat ($68) — add R14:R15 to R8:R9, saturate at $FFFF"),
    0x06CB: ("op_add_r8r9_to_1617", "AB+=ENV ($74) — add R8:R9 to R16:R17"),
    0x06D4: ("op_add_1415_to_1617", "AB+=CD ($6C) — add R14:R15 to R16:R17"),
    0x06DD: ("op_add_r8r9_to_1415", "CD+=ENV ($78) — add R8:R9 to R14:R15"),
    0x06E6: ("op_add_1617_to_1415", "CD+=AB ($7C) — add R16:R17 to R14:R15"),
    0x06EF: ("op_sub_1617_from_r8r9", "ENV-=AB ($84) — subtract R16:R17 from R8:R9"),
    0x06F8: ("op_sub_1415_from_r8r9", "ENV-=CD ($88) — subtract R14:R15 from R8:R9, floor at 0"),
    0x0708: ("op_sub_r8r9_from_1617", "AB-=ENV ($94) — subtract R8:R9 from R16:R17"),
    0x0711: ("op_sub_1415_from_1617", "AB-=CD ($8C) — subtract R14:R15 from R16:R17"),
    0x071A: ("op_sub_r8r9_from_1415", "CD-=ENV ($98) — subtract R8:R9 from R14:R15"),
    0x0723: ("op_sub_1617_from_1415", "CD-=AB ($9C) — subtract R16:R17 from R14:R15"),
    # Negate + multiply ($072C-$074C)
    0x0736: ("op_neg14_multiply", "ENV*=0.(-C) ($F4) — negate R14_HI then multiply ENV"),
    0x0740: ("op_mul_by_16", "ENV*=0.A ($D0) — multiply ENV by R16_HI (fractional)"),
    0x0744: ("op_mul_by_11", "ENV*=0.C ($D4) — multiply ENV by R14_HI (fractional)"),
    0x0748: ("op_mul_by_imm", "ENV*=0.imm ($B4) — multiply ENV by immediate (fractional)"),
    # Multiply variants ($074D-$07DC)
    0x076C: ("op_neg14_mul_to_1617", "AB*=0.(-C) ($F8) — negate R14_HI, multiply into R16:R17"),
    0x0776: (".neg14_zero_fallback", None),  # UNREACHABLE without $0773 patch ($8D→$AD)
    0x077A: ("op_mul14_to_1617", "AB*=0.C ($D8) — multiply R16:R17 by R14_HI (fractional)"),
    0x077E: ("op_mulimm_to_1617", "AB*=0.imm ($B8) — multiply R16:R17 by immediate (fractional)"),
    0x0783: ("multiply_to_1617", None),
    0x07A4: ("op_neg16_mul_to_1415", "CD*=0.(-A) ($FC) — negate R16_HI, multiply into R14:R15"),
    0x07AE: (".neg16_zero_fallback", None),  # UNREACHABLE without $07AB patch ($8D→$AD)
    0x07B2: ("op_mul14_to_1415", "CD*=0.C ($DC) — multiply R14:R15 by R14_HI (fractional)"),
    0x07B6: ("op_mulimm_to_1415", "CD*=0.imm ($BC) — multiply R14:R15 by immediate (fractional)"),
    0x07BB: ("multiply_to_1415", None),
    0x07DC: ("op_mul16_acc_r8r9", "ENV*=1.A ($E0) — multiply ENV by 1+R16_HI"),
    0x07E0: ("op_mul14_acc_r8r9", "ENV*=1.C ($E4) — multiply ENV by 1+R14_HI"),
    0x07E4: ("op_mulimm_acc_r8r9", "ENV*=1.imm ($C4) — multiply ENV by 1+immediate"),
    # Multiply-accumulate to reg pairs
    0x080B: ("op_mul14_acc_1617", "AB*=1.C ($E8) — multiply R16:R17 by 1+R14_HI"),
    0x080F: ("op_mulimm_acc_1617", "AB*=1.imm ($C8) — multiply R16:R17 by 1+immediate"),
    0x083C: ("op_mul16_acc_1415", "CD*=1.A ($EC) — multiply R14:R15 by 1+R16_HI"),
    0x0840: ("op_mulimm_acc_1415", "CD*=1.imm ($CC) — multiply R14:R15 by 1+immediate"),
    ...
    0x089D: ("op_mul_1415_by_imm", "CD=0.imm*(C+D) ($C0) — fractional scale of R14:R15 sum"),
    # Comparison + conditional branch ($08CD-$0965)
    # mk1envlib: IF X<Y GOTO / IF X>Y GOTO — signed/unsigned compare + branch
    0x08D1: ("cmp_u_r8r9_imm", "IF ENV<imm GOTO ($xD) — unsigned compare ENV vs immediate"),
    0x08D5: ("cmp_s_1617_imm", "IF AB>imm GOTO ($xA) — signed compare AB vs immediate"),
    0x08D9: ("cmp_u_1617_imm", "IF AB<imm GOTO ($xE) — unsigned compare AB vs immediate"),
    0x08DD: ("cmp_s_1415_imm", "IF CD>imm GOTO ($xB) — signed compare CD vs immediate"),
    0x08E1: ("cmp_u_1415_imm", "IF CD<imm GOTO ($xF) — unsigned compare CD vs immediate"),
    0x08E5: ("cmp_u_r8r9_1617", "IF ENV<AB GOTO ($24) — unsigned compare ENV vs AB"),
    0x08ED: ("cmp_u_r8r9_1415", "IF ENV<CD GOTO ($28) — unsigned compare ENV vs CD"),
    0x08F5: ("cmp_u_1617_1415", "IF CD<AB GOTO ($2C) — unsigned compare CD vs AB"),
    0x08FD: ("cmp_u_1617_r8r9", "IF ENV>AB GOTO ($34) — unsigned compare AB vs ENV"),
    0x0905: ("cmp_u_1415_1617", "IF CD>AB GOTO ($3C) — unsigned compare AB vs CD"),
    0x090D: ("cmp_u_1415_r8r9", "IF ENV>CD GOTO ($38) — unsigned compare CD vs ENV"),
    # Loop control reload variants
    0x09F9: ("op_loop_reload_18_from_16", "LOOP0 N=A ($08) — primary loop, count from R16_HI"),
    0x0A06: ("op_loop_reload_19_from_16", "LOOP1 N=A ($18) — secondary loop, count from R16_HI"),
    0x0A13: ("op_loop_reload_18_from_14", "LOOP0 N=C ($0C) — primary loop, count from R14_HI"),
    0x0A20: ("op_loop_reload_19_from_14", "LOOP1 N=C ($1C) — secondary loop, count from R14_HI"),
    # Negate + special
    0x0A72: ("op_neg_1415", "CD=-CD ($50) — negate R14:R15 (two's complement)"),
    0x0A7B: ("op_stdlinear", "STDLINEAR ($A0) — 6-stage linear envelope, 21-byte instruction"),
    0x0A85: (".stage_resume", None),
    0x0A8D: (".load_duration", None),
    0x0A95: (".check_sustain", None),
    0x0A9A: (".check_done", None),
    0x0AA7: (".advance_stage", None),
    0x0AB2: (".all_stages_done", None),
}
...
ADDR_COMMENTS = {
    ...
    # synthesis_mode_c ($0412): compute wrap-point divisor + fill wavetable
    0x0412: [
    ...
    0x0414: "R14 = sram[$F7] = freq step (temp: used in division)",
    0x0418: "R5 = sram[$F6] = waveform source offset",
    0x041A: "R5 += $3F → point to last sample in waveform area",
    0x041D: "R4 = $10 (set up RR4 = $10:R5 for slave RAM reads)",
    0x041F: "R10 = sram[R4:R5] = last waveform sample",
    0x0421: "R11 = 0 (extend to 16-bit for division)",
    0x0423: "R10 -= R14 (last_sample - freq_step)",
    0x0425: "RCF; RRC R10:R11 → divide by 2",
    0x042A: "R10 += 1",
    0x042B: "R10 += R15 (add wrap point)",
    0x042D: "RRC R10:R11 → divide by 2 again (dividend ready)",
    0x0431: "R13 = 0 (divisor low byte)",
    0x0433: "R12 = $41 + R15 (divisor base)",
    0x0437: "R14 = 0 (quotient accumulator)",
    0x0439: "RRC R12:R13 → divisor / 2 (first bit position)",
    0x043D: "--- 8-bit restoring division: 8 unrolled iterations ---",
    0x04B7: "last RLC R14 (8th quotient bit)",
    0x04B9: "COM R14 → complement quotient (invert all bits)",
    0x04BB: "R13 = $3E (index for reg[$7F] = $41 + $3E)",
    0x04BD: "reg[$7F] = R14 (store wrap-boundary value at end of wavetable)",
    0x04C0: "R12 = next sample from sram (read via RR4, R5 decrements)",
    0x04C2: "R14 -= R12 (delta = wrap_value - sample)",
    0x04C4: "DEC R5 (advance source pointer backward)",
    0x04C6: "fill loop: R12 = sram sample",
    0x04C8: "R12 += R14 (add delta offset to sample)",
    0x04CA: "reg[$40+R13] = R12 (store in wavetable)",
    0x04CD: "DEC R5 (next source sample)",
    0x04CF: "DJNZ R13 → loop until reg[$41] filled",
    0x04D1: "last sample for reg[$40]",
    0x04D8: "R10 = $10 (restore slave RAM bank byte for RR10)",
    0x04DA: "→ synthesis_loop_entry",
    # synthesis_mode_b ($04DD): waveform load into reg[$40-$7F]
    0x04DD: [
    ...
    0x04E1: "R11 = sram[$F6] = waveform source address in slave RAM",
    0x04E5: "R14 == 0? (no delta subtraction needed?)",
    0x04E8: "R14 == 0 → direct LDEI copy path",
    0x04EA: "R11 += R15 → point to end of source (loop copies backward)",
    0x04EC: "R13 = R15 (loop counter = sample count)",
    0x04EE: "R12 = sram[R11] (read one sample from slave RAM)",
    0x04F0: "R12 -= R14 (subtract DC offset / delta)",
    0x04F2: "reg[$40+R13] = R12 (store processed sample in wavetable area)",
    0x04F5: "DEC R11 (move to previous source byte)",
    0x04F7: "DJNZ R13 → loop (R13 counts down, stores at $40+R13...$40+1)",
    0x04F9: "last sample: R12 = sram[R11]",
    0x04FB: "R12 -= R14",
    0x04FD: "reg[$40+0] = R12 (store at reg[$40], completing the table)",
    0x0500: "→ synthesis_loop_entry",
    0x0502: "direct copy path (R14 == 0): use LDEI sled",
    0x0504: "R5 = $DD (end of LDEI sled)",
    0x0506: "R5 -= R15 * 2 (compute sled entry for R15 bytes)",
    0x050A: "CALL @RR4 → LDEI sled copies R15 samples: sram[R11+] → reg[$40+]",
    0x050C: "→ synthesis_loop_entry",
    # update_path: RAUD default handler — no command bits set
    # Reloads micro-program pointer and pitch envelope from slave RAM,
    # then restarts the micro-program interpreter.
    0x0330: [
    ...
    0x0332: "R11 = sram[$F5] = micro-program source base (e.g., $D4)",
    0x0334: "INC R11 → point to sram[source+1] (saved PROG_CTR start)",
    0x0335: "R12 = sram[source+1] = saved start offset ($02 after first_time)",
    0x0337: "PROG_CTR = saved start offset (relative, before ADD #$1D)",
    0x0339: "R11 = $FE (slave RAM address of reg[$14] init)",
    0x033B: "R12 = $14 (destination register = ACC2_HI)",
    0x033D: "reg[$14] = sram[$FE] (ACC2_HI), R12→$15, R11→$FF",
    0x033F: "reg[$15] = sram[$FF] (ACC2_LO), R12→$16, R11→$00(wrap)",
    0x0341: "R12 = Port 3 (EXSLA routing bits from master)",
    0x0343: "P01M = $0C (Port 0 = output, Port 1 = input; release data bus)",
    0x0346: "XOR Port 2 (toggle bus handshake lines)",
    0x0349: "P2M = $04 (RARC = input/high-Z; release bus to master)",
    0x034C: "MODE bit 7: pitch envelope enable?",
    0x034F: "bit 7 SET (Z) → pitch tracking active: load PITCH_ENV + EXSLA",
    0x0351: "bit 7 CLEAR: fixed formant, no pitch modulation",
    0x0354: "skip to loop counter reset",
    0x0356: "PITCH_ENV = ACC2_LO (pitch modulation depth from COP)",
    0x0359: "clear ACC2_LO after consuming it",
    0x035B: "extract EXSLA routing: SWAP + RL + AND $C0 → bits [7:6]",
    0x0362: "clear EXSLA bits in PITCH_HI (keep coeff bits [5:0])",
    0x0365: "merge new EXSLA bits [7:6] into PITCH_HI",
    0x0368: "clear loop counters (micro-program loops restart each RAUD)",
    0x036C: "PROG_CTR += $1D → absolute register address (reg[$1F] for offset $02)",
    0x036F: "clear block counter (LFSR/repeat state resets each RAUD)",
    0x0371: "restart micro-program interpreter from saved position",
    # irq3_full_setup ($03A5): bulk-load sram[$F9-$FF] → reg[$0F-$15]
    0x03A5: [
    ...
    0x03A9: "FLAGS = R13 (save command byte for irq3_first_time check)",
    0x03AB: "R13 = (R13 & $F8) + $02 → slave address for bus handshake",
    0x03B1: "PORT2 = R13 (assert slave select on bus)",
    0x03B3: "R11 = $F9 (LDEI source start: sram[$F9])",
    0x03B5: "R12 = $0F (LDEI dest start: reg[$0F] = R15)",
    0x03B7: "sram[$F9] → reg[$0F] = R15: wrap point (Mode C) / sample count (Mode B)",
    0x03B9: "sram[$FA] → reg[$10] = MODE: mode bits 5:4 + sub bits 3:0",
    0x03BB: "sram[$FB] → reg[$11] = LFSR_VIB_STATE: used for ACC3_HI init, then overwritten to $DB",
    0x03BD: "sram[$FC] → reg[$12] = PITCH_HI: coeff high (bits 3:0 = multiply, 7:6 = EXSLA)",
    0x03BF: "sram[$FD] → reg[$13] = PITCH_LO: coeff low",
    0x03C1: "sram[$FE] → reg[$14] = ACC2_HI: pitch envelope param",
    0x03C3: "sram[$FF] → reg[$15] = ACC2_LO: pitch fine value (from voice_write_pitch)",
    0x03C5: "ACC3_HI = sram[$FB] (move to reg[$16] — this is the actual use of sram[$FB])",
    0x03C8: "ACC3_LO = 0",
    0x03CA: "LFSR_VIB_STATE = $DB (fixed init seed for LFSR/vibrato)",
    0x03CD: "MODE bit 7: pitch envelope enable?",
    0x03D0: "bit 7 SET (Z) → pitch tracking active: load PITCH_ENV + EXSLA",
    0x03D2: "bit 7 CLEAR: fixed formant, no pitch modulation",
    ...
    0x03FD: "PROG_CTR = reg[$1D] (first micro-program byte = start_offset / library address)",
    # synthesis_loop_entry: prepare interpreter state and add base to PROG_CTR
    0x057B: [
    ...
    0x057D: "ACC_LO = 0",
    0x0585: "R4 = 0 (ZERO register, used throughout as constant 0)",
    0x0587: "MICRO_OP = $19 (silence micro-op, placeholder until real synthesis starts)",
    0x0589: "ADD $1C to PROG_CTR (+ INC at $058C = +$1D converts offset to reg address)",
    ...
    # op_stdlinear ($A0): STDLINEAR — 6-stage linear envelope
    0x0A7B: [
    ...
    0x0A7D: "BLOCK_CTR == 0? (first call or after RAUD reset)",
    0x0A80: "not first → use existing BLOCK_CTR (resume current stage)",
    0x0A82: "first call: BLOCK_CTR = 3 (point to stage 1 duration)",
    0x0A85: "R10 = PROG_CTR + BLOCK_CTR → current stage's duration byte",
    0x0A88: "LOOP1_CTR == 0? (need to load new stage?)",
    0x0A8B: "LOOP1_CTR != 0 → check sign and continue current stage",
    0x0A8D: "load duration: LOOP1_CTR = @R10 (stage's duration byte)",
    0x0A93: "jump to zero-check (skip the MI/DEC path)",
    0x0A95: "LOOP1_CTR < 0 (bit 7 set) → sustain: hold at multiply forever",
    0x0A98: "DEC LOOP1_CTR (count down one ECLK tick)",
    0x0A9A: "LOOP1_CTR == 0 after DEC or load? → advance to next stage",
    0x0A9C: "DEC R10 → points to step_lo",
    0x0A9E: "ACC_LO += step_lo",
    0x0AA0: "DEC R10 → points to step_hi",
    0x0AA2: "ACC_HI += step_hi (with carry from step_lo add)",
    0x0AA4: "→ multiply with updated ACC",
    0x0AA7: "advance to next stage: BLOCK_CTR += 3, R10 += 3",
    0x0AAD: "BLOCK_CTR <= $12 (18)? → more stages remain, loop back",
    0x0AB2: "all 6 stages done: PROG_CTR = R10 (advance past 21-byte instruction)",
    0x0AB4: "load final ACC value from bytes[19:20]",
    0x0ABC: "clear BLOCK_CTR and LOOP1_CTR (envelope complete)",
    0x0AC0: "→ multiply with final ACC value",
    0x0AC3: [
    ...
    0x0B0A: "save R8 to LFSR_VIB_STATE (reg[$11])",
    ...
    0x0BD6: "MODE bit 7: pitch envelope enable? (same test as update_path/$034C)",
    0x0BD9: "bit 7 CLEAR → no pitch tracking, skip PITCH_ENV/EXSLA update",
    ...
    0x0CA7: "which library entry? compare PROG_CTR to thresholds",
    0x0CAA: "PROG_CTR > $43 → check upper range ($2A/$2E entries)",
    0x0CAC: "PROG_CTR == $43: library $26 (ENV=$4000)",
    0x0CAE: "PROG_CTR < $43: library $22 (ENV=$2D00 ≈ $2CE4)",
    0x0CB2: "library $26: ACC = $4000 (quarter range)",
    0x0CB6: "check if PROG_CTR >= $4B (library $2E)",
    0x0CB9: "PROG_CTR >= $4B → library $2E (ENV=$FFFF)",
    0x0CBB: "library $2A: ACC = $8000 (center — standard for noise voices)",
    0x0CBD: "clear low byte (ACC = ACC_HI:$00)",
    0x0CC1: "library $2E: ACC = $FFFF (maximum)",
    0x0CC5: "PROG_CTR = $3D → HALT state (library program done, just multiply from now on)",
        # overflow_shift
    0x0CCC: [
    ...
    0x0E62: [
        ...
        "",
        "LOWER PATH (T<$40 → 1x DDS mode A sub 8+ or mode B/C):",
    ...
    0x0E1C: "not $F2 → skip gate (only Mode C needs this check)",
    0x0E1E: "R12 = $01 (ROM high byte for param table at $01xx)",
    0x0E20: "R12 = ROM[$01:R13] = T byte = phase step for new sub-mode",
    0x0E22: "RL R12 → T * 2 (phase advance in 2 IRQ4 ticks)",
    0x0E24: "T*2 vs R15: C set if T*2 < R15 (at least 2 samples/cycle)",
    0x0E26: "T*2 < R15 → below Nyquist, normal operation",
    0x0E28: "T*2 >= R15 → above Nyquist! R13=$2E = silence (Mode C sub=15, op=$19)",
    ...
    0x0E4A: "MODE bit 7: pitch envelope enable? (bit 7 CLEAR = no pitch tracking)",
...
MEM_REG_ALIASES = {
    ...
    0x11: ("LFSR_VIB_STATE",    "LFSR high byte / vibrato running state"),
